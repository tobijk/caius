...
* `method send_keys string`:
  Send the characters in the given `string` as key strokes to the element.
...
* `method set_expiry timestamp`:
  Set the expiration date of the cookie. `timestamp` is the number of seconds since
  Jan 1, 1970 UTC.
...
`Capabilities` are use to request session properties during initialization of
a `WebDriver::Session`.
...
* `method browser_connection_enabled`:
  Determine whether the session can query for the browser's connectivity and
...
* `method set_proxy [WebDriver::Proxy]`:
